...
public sealed class ResolutionQueue
{
        ...
        out string error)
    {
        ...
        decision.TriggerEvent = GameEventSnapshot.FromRuntime(triggerEvent);
    ...
    public bool TrySubmitDecision(string playerId, string decisionId, IEnumerable<int> selectedIds, out PendingDecisionSnapshot continuation, out string error)
    {
        ...
        HashSet<int> candidates = new HashSet<int>(decision.CandidateInstanceIds);
        HashSet<int> unique = new HashSet<int>();
        foreach (int id in selected)
        {
            if (!candidates.Contains(id) || !unique.Add(id)) { error = "Selection contains an invalid or duplicate card."; return false; }
        }
    ...
    private static PendingDecisionSnapshot CloneDecision(PendingDecisionSnapshot source)
    {
        PendingDecisionSnapshot clone = new PendingDecisionSnapshot
        {
            ...
            ListenerCardInstanceId = source.ListenerCardInstanceId,
    ...
    public static void EnsureSnapshot(GameStateSnapshot state)
    {
        ...
        if (state.Resolution.SelectedInstanceIds == null)
            state.Resolution.SelectedInstanceIds = new List<int>();
    }
}
